...
* link:#client[ntp.conf (you are the client)]
* link:#server[ntp.conf (you are the server)]
...
The RFC hasn't been published yet (Dec 2019).  Nothing has changed
...
spent debugging.  The document link:quick.adoc[Quick Start] may
be helpful.
...
Dec 2019, the following should work:
...
This assumes that the remote server is using a certificate covered by
...
# Signal ntpd to pick up new certificate(s)
...
* Your certificate Public Key, fully chained up
