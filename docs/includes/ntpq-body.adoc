...
the supplied starting points newer neighbor, fetch the supplied
entries. This enables fetching multiple entries from given IP
addresses (provided by addr.x= entries where 0 <= x <= 15).
When limit is not one and frags= is provided,
