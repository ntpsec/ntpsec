= Trimble Palisade/Thunderbolt/Acutime/Resolution/ACE/Copernicus & Praecis Receivers
...
Acutime Gold, Resolution, ACE III and Copernicus II.
...
The driver has been tested with the Resolution, ACE and Copernicus
receivers on a Rapsberry Pi 3B running Raspberry Pi OS (Legacy).
...
[width="50%",cols="<40%,<80%",options="header"]
|=================================================
|OS              | Command
...
All receivers except the Thunderbolt and Copernicus II have a factory default serial port
configuration of 8O1 (odd parity). The Thunderbolt and Copernicus II default to 8N1 (no parity).
All receivers except the Copernicus II have a factory default serial port
speed of 9600 baud. The Copernicus II defaults to 38400 baud.
...
Resolution:
---------------------------------------------
refclock trimble unit 0 subtype 5 time1 0.420
---------------------------------------------

ACE III:
---------------------------------------------
refclock trimble unit 0 subtype 6
---------------------------------------------

Copernicus II:
---------------------------------------------
refclock trimble unit 0 subtype 7 time1 0.140
---------------------------------------------
...
== Initial Setup and Testing for Resolution Receivers
1.  Read the xref:Res[Resolution] notes.
...
== Initial Setup and Testing for ACI III Receivers
1.  Read the xref:ACE[ACI III] notes.
2.  Place the GPS antenna outdoors, with a clear view of the sky for best
    results -- ACI III is not very good at tracking weak signals.
3.  Power up and allow the receiver to obtain UTC offset data. This can take
    13 to 30 minutes.
4.  Configure the serial I/O port and its xref:sym[symbolic link] on the host.
5.  Add the refclock to your +ntpd+ xref:cfg[configuration file].
...
[[t1]]+time1+ 'time'::
  ...
  Thunderbolt, 420ms when using a Resolution and 140ms when using a
  Copernicus II.
...
+flag2 {0 | 1}+::
  ...
  receivers except the Thunderbolt, Resolution and Copernicus II.
...
[[f3]]+flag3 {0 | 1}+::
  ...
  Resolution, ACE III or Copernicus II.
...
The driver will attempt to set the time report packet and PPS output alignment
to UTC time since the Resolution defaults to GPS alignment. Time transfer will
not be allowed unless the receiver reports that it is aligned to UTC time.  The
driver will also set the receiver not to output a PPS unless at least one
satellite is being received.

The Resolution does not have an event input.  The driver therefore uses the
timecode packets transmitted by the Resolution after the beginning of each
...
The Resolution will have a "GPS Week Number Rollover" problem after
the following dates:
[width="50%",cols="<50%,<50%",options="header"]
|=============================
...
xref:Pal[Palisade / Acutime Notes] section is implemented for the Resolution.
...
4.00 +
* add support for Resolution, ACE III and Copernicus II receivers +
