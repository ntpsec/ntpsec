= Trimble Palisade/Thunderbolt/Acutime/Resolution 360/ACE/Copernicus & Praecis Receivers
...
Acutime Gold, Resolution 360, ACE III and Copernicus II.
...
[width="60%",cols="<60%,<40%",options="header"]
|============================================
...
| Resolution 360 SMT 66974-45 | 1.14
...
The driver has been tested with the Resolution 360, ACE and Copernicus
...
== Initial Setup and Testing for Resolution 360 Receivers
1.  Read the xref:Res[Resolution 360] notes.
...
[[t1]]+time1+ 'time'::
  ...
  Thunderbolt, 420ms when using a Resolution 360 and 140ms when using a
  Copernicus II.
...
+flag2 {0 | 1}+::
  ...
  receivers except the Thunderbolt, Resolution 360 and Copernicus II.
...
[[f3]]+flag3 {0 | 1}+::
  ...
  Resolution 360, ACE III or Copernicus II.
...
[[Res]]
== Resolution 360 Notes

The driver will attempt to set the time report packet and PPS output alignment
to UTC time since the Resolution 360 defaults to GPS alignment. Time transfer will
...
The Resolution 360 does not have an event input.  The driver therefore uses the
timecode packets transmitted by the 360 Resolution after the beginning of each
...
The Resolution 360 will have a "GPS Week Number Rollover" problem after
the following dates:
[width="50%",cols="<50%,<50%",options="header"]
|=============================
...
xref:Pal[Palisade / Acutime Notes] section is implemented for the Resolution 360.
...
4.00 +
* add support for Resolution 360, ACE III and Copernicus II receivers +
