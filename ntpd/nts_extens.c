...
int extens_client_send(struct peer *peer, struct pkt *xpkt) {
        ...
        ex_append_record_bytes(&buf, NTS_Cookie,
                               peer->nts_state.cookies[idx],
                               peer->nts_state.cookielen);
        ...
        for (int i=peer->nts_state.count+1; i<NTS_MAX_COOKIES; i++) {
                ...
                ex_append_header(&buf, NTS_Cookie_Placeholder,
                                 peer->nts_state.cookielen);
        ...
        ex_append_header(&buf, NTS_AEEF,
                         NTP_EX_U16_LNG*2+NONCE_LENGTH+CMAC_LENGTH);
        ...
        if (!ok) {
                msyslog(LOG_ERR,
                        "NTS: extens_client_send - Error from AES_SIV_Encrypt");
        ...
        while (buf.left >= NTS_KE_HDR_LNG) {
                ...
                switch (type) {
                    ...
                    case NTS_Cookie:
                        ...
                        memcpy((uint8_t*)&peer->nts_state.cookies[idx],
                               buf.next, length);
                        peer->nts_state.writeIdx = peer->nts_state.writeIdx %
                                                   NTS_MAX_COOKIES;
                        peer->nts_state.count++;
                        buf.next += length;
                        buf.left -= length;
